...
void print_history(void) {
  ...
  int k;
  for ( k = 0; k < n; k++) {
      printf(1, "%d %s %d\n", hist[k].pid, hist[k].name, hist[k].memory_usage);
  }
}
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if (buf[0] == 'b' && buf[1] == 'l' && buf[2] == 'o' && buf[3] == 'c' && buf[4] == 'k' && buf[5] == ' ') {
        ...
        // Implement "unblock" command character-wise
...
if (buf[0] == 'h' && buf[1] == 'i' && buf[2] == 's' &&
      ...
      buf[3] == 'o' && buf[4] == 'd' && buf[5] == ' ') 
  {
      ...
      // Find the first space after "chmod "
      char *filename = buf + 6;
      char *mode_str = strchr(filename, ' ');

      if (!mode_str) {
          printf(2, "chmod: missing mode argument\n");
          continue;
      }

      *mode_str = 0; // Split the string into filename and mode
      mode_str++;     // Move to the mode part

      int mode = atoi(mode_str);
      if (mode < 0 || mode > 7) {
          printf(2, "chmod: invalid mode %d, should be between 0-7\n", mode);
          continue;
      }

      // Call the chmod syscall
      if (chmod(filename, mode) < 0)
          printf(2, "chmod: failed to change permissions for %s\n", filename);
      else
          printf(1, "chmod: changed permissions of %s to %d\n", filename, mode);
